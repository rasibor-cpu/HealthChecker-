...
Additionally, 5 pre-existing tests with stale hardcoded Android version literals (`versionCode = 324`/`321`, `versionName = "0.324.0"`) were fixed to track the actual current release (`versionCode = 327`, `versionName = "0.327.0"`) rather than a frozen historical value — see Section 24.
...
**Confirmed clean (no findings) in:** authentication server-side enforcement (Section 12), password lifecycle (Section 12), recovery challenge verification and anti-enumeration (Section 13), patient isolation on all primary API surfaces (Section 14), device pairing/revocation on the main app (Section 14), Android production-origin lock (Section 16), SAF content-URI handling (Section 17, one item NOT_YET_PROVEN by device test, not a defect), vault encryption/fail-closed behavior/key handling (Section 15), backup exclusion of key material (Section 15), debug/release build separation (`ALLOW_CLEARTEXT_LOCAL_DEV` gated by `BuildConfig.DEBUG`, Section 16), secrets hygiene (no committed keys/certs, Section 15), and log redaction (Section 15). TLS/public-origin assumptions rely on the existing Cloudflare/reverse-proxy layer, which is out of scope for source review and was not touched (per Section 4 safety rules).
...
- `isSafContentUri()` requires `scheme == "content"` + non-blank authority — `file://` and malformed URIs are rejected; covered by `ConsumerSafFileChooserPolicyTest.fileSchemeAndMissingAuthorityAreRejected`.
...
| Field | Value |
|---|---|
| versionCode | 327 (unchanged — no Android application-source-set code changed in this closeout; only a test-source-set relocation and unrelated backend/Python fixes) |
| versionName | "0.327.0" |
| APK path | `android/app/build/outputs/apk/release/app-release-unsigned.apk` (local build output only, not distributed) |
| APK size | 3,979,279 bytes |
| APK SHA256 (this unsigned validation build) | `4022f75f170955dd20c103826584a7459379c0c99c99c6cd7c24f60aa1b8c929` |
...
| Build test result | `testDebugUnitTest`: **168 tests, 0 failures, 0 errors, 0 skipped.** `testReleaseUnitTest`: **156 tests, 0 failures, 0 errors, 0 skipped** (12 fewer than debug — exactly the 3 test files now correctly scoped to the debug-only source set, as intended). |
...
| Final regression suite passes | **PASS** | Section 24 — 1495 total, 1492 passed, 0 failed, 3 skipped (all environment-conditional, documented), 0 errors |
