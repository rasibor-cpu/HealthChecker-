...
| D. Android SAF record import | **Real defect found via device UAT, now FIXED in source** (`ENGINEERING_SOURCE_CLOSEOUT = PASS`), `FINAL_DEVICE_ACCEPTANCE = WAITING_FOR_OPERATOR` (retest pending) | See Section 17. Physical-device UAT surfaced "Failed to fetch" on upload — most likely/proximate cause (established by device symptom and source-path elimination, not directly observed via trace): Chromium WebView's unreliable handling of `content://`-backed Blobs in `fetch()` multipart bodies. Fixed with a narrowly scoped native read bridge (`ConsumerRecordImportBridge`) that removes that code path regardless of the exact internal mechanism; all prior security properties re-verified intact. A fresh device UAT is required to confirm the fix on-device — see Section 26 for the required in-place APK update. |
...
3. **SAF record-upload fix — real defect found via physical-device UAT** ([android/app/src/main/java/com/healthchecker/companion/consumer/ConsumerRecordImportBridge.kt](android/app/src/main/java/com/healthchecker/companion/consumer/ConsumerRecordImportBridge.kt), new file; [ConsumerSafFileChooserPolicy.kt](android/app/src/main/java/com/healthchecker/companion/consumer/ConsumerSafFileChooserPolicy.kt); [ConsumerLauncherActivity.kt](android/app/src/main/java/com/healthchecker/companion/ui/ConsumerLauncherActivity.kt); [js/health_vault/mobile_consumer.js](js/health_vault/mobile_consumer.js)) — the operator's device UAT surfaced "Failed to fetch" when uploading a SAF-selected file on the physical S24. Most likely/proximate cause, established by the device symptom plus elimination of every other candidate via source inspection (no WebView trace/logcat was captured — see Section 17's evidence-discipline note): Android WebView's Chromium engine cannot reliably stream a `content://`-backed file into a `fetch()`/`FormData` multipart body. Fixed by reading the selected document's bytes natively via `ContentResolver` and handing them to JavaScript through one new, narrowly scoped `@JavascriptInterface` bridge that takes no URI/path parameter from JS (it can only ever read whatever the native file picker most recently recorded) — a fix that removes the entire suspect code path regardless of the precise internal Chromium mechanism. All previously-verified security properties (no `file://`, no broad storage permission, narrow content-URI scope, production origin lock, authenticated upload, no secret logging) were re-confirmed intact. See Section 17 for the full analysis and hypothesis ruling.
...
### Root cause — most likely/proximate cause established by device symptom and source-path elimination, not directly observed

**Evidence discipline note:** no WebView/Chromium debug trace, logcat capture, or network inspector session was collected during the device UAT — `WebView.setWebContentsDebuggingEnabled(false)` is deliberately set in the release build (Section 16), so this app cannot be attached to Chrome DevTools in its normal configuration, and no such capture was requested or performed. What follows is the most likely/proximate cause established by (a) the exact device symptom reported and (b) elimination of every other candidate failure point via source inspection (see the hypothesis table below) — not a claim that Chromium's internal `content://` blob-read failure was directly observed in a trace. If direct WebView/logcat/network evidence is obtained later that confirms or contradicts this, this section should be updated accordingly.

"Failed to fetch" is the literal text of the browser's own native `TypeError` thrown by `fetch()` when a request fails at the network/transport layer *before* any HTTP response is received — confirmed by grepping the entire repository for that exact string: it appears nowhere in this codebase's own error-handling code (only once, in an unrelated code comment referencing a prior, different incident), which is direct evidence that our own code never manufactures this text — it can only be the browser's own message. The app's `upload()` function (`js/health_vault/mobile_consumer.js`) built the multipart upload body directly from the `<input type=file>`'s `File` object, which the WebView populates from the `content://` URI returned by the SAF picker. Given that (1) the literal error text is confirmed browser-native, (2) every other candidate failure point in the request path was independently re-verified correct and ruled out by reading its actual source (auth headers, endpoint path, CORS, origin lock, server-side handling — see the table below), and (3) Android WebView's Chromium engine has a documented history of being unreliable at streaming/reading a `content://`-backed `File`/Blob when serializing it into a `fetch()` multipart body, this is assessed as the most likely/proximate cause — not a directly observed one. It is consistent with the reported symptom (an initial delay before the error surfaced, matching a stream read that eventually times out or fails rather than failing instantly), but that timing detail is circumstantial support, not proof of the specific internal Chromium failure mode. The fix (native byte read, bypassing the WebView's content:// blob path entirely for this request) is appropriate regardless of the precise internal mechanism, since it removes the entire class of "WebView reads a content:// blob for the multipart body" from the request path.

### Hypotheses ruled in/out (explicit, as required)
| # | Hypothesis | Verdict | Evidence |
|---|---|---|---|
| A | content:// URI cannot be read from WebView/JS | **Most likely, as the failure class** (not directly observed — no trace evidence) | The URI itself is readable by native `ContentResolver` (proven by the new bridge/tests below), so the read succeeds outside the WebView; the failure is assessed as most likely at Chromium's fetch()-body-serialization boundary specifically, based on elimination of every other candidate below, not on a captured trace of that boundary failing. |
...
| C | File picker returns metadata but not readable bytes | **Consistent with the reported symptom, not independently confirmed** | Filename/selection succeeding while the byte read at upload time fails is consistent with what was reported, but this was not captured in a trace — it's an inference from the symptom description. |
| D | FormData/File object creation from content:// URI is invalid | **Most likely, as a contributing factor** (not directly observed) | The `<input>`'s File object is valid metadata-wise (selection succeeded); its Chromium-internal blob backing is assessed as the unreliable part by elimination, not by direct inspection of Chromium's blob implementation. |
| E | WebView's fetch() cannot directly dereference content:// | **Most likely — the assessed proximate mechanism, not directly observed** | Consistent with widely documented Android WebView/Chromium behavior for `content://`-backed Blob uploads; no logcat/network trace from this specific device run confirms it was this exact mechanism rather than a related one. |
| F | Android bridge expected to convert content:// into bytes but does not | **Confirmed as a real gap, now closed** (this one is directly verifiable from source, not an inference) | No `@JavascriptInterface` existed anywhere in the app before this fix — confirmed by repo-wide grep, a direct source fact rather than a device-behavior inference. |
...
### Hardening pass (this revision) — three issues found on final review of the bridge itself

**1. The initial bridge implementation did not actually bound native memory consumption.** `openInputStream(...).use { it.readBytes() }` fully allocated the document before comparing its size against `MAX_IMPORT_BYTES` — a huge or hostile document would be fully buffered regardless. Fixed with `ConsumerSafFileChooserPolicy.readBounded(input, limit)`: a pure function that allocates exactly one `limit + 1`-byte buffer and reads into it, stopping the instant that buffer fills — it never buffers more than `MAX_IMPORT_BYTES` (15MB) + 1 byte, regardless of how large the underlying stream actually is or claims to be. A document's `OpenableColumns.SIZE` (when a provider reports one) is consulted only as an early-rejection *optimization* to skip opening a stream that's already known to be oversized — per the explicit instruction, this is never the enforcement mechanism, since declared size can be absent or wrong; the streaming bound in `readBounded()` is authoritative regardless. No temp files are used at any point — `ContentResolver`-only access is preserved.
   - Proof of the bound: `ConsumerRecordImportBridgeTest.c_streamLargerThanLimitIsNotFullyConsumed` feeds `readBounded()` a custom `InputStream` that never terminates (would loop forever if fully consumed) and asserts it stops after serving exactly `limit + 1` bytes.
   - Tests A-E (as specified): exactly `MAX_IMPORT_BYTES` succeeds (`a_exactlyMaxImportBytesSucceeds`); `MAX_IMPORT_BYTES + 1` fails (`b_maxImportBytesPlusOneFails`); a larger-than-limit stream is not fully consumed (`c_streamLargerThanLimitIsNotFullyConsumed`, above); unknown-size streams (no `OpenableColumns.SIZE` provider registered at all) are still correctly bounded (`d_unknownSizeStreamsAreStillBounded`); an unreadable stream fails safely without throwing (`e_unreadableUriReturnsAnErrorWithoutThrowing`). An additional test (`declaredOversizeShortCircuitsWithoutOpeningTheStream`, via a minimal fake `ContentProvider`) proves the declared-size fast path skips opening a stream at all when the provider does report an oversized document.

**2. The selected URI remained readable through the bridge indefinitely after a successful read.** Reduced exposure: `ConsumerRecordImportBridge` now takes an `onConsumed: () -> Unit` callback in addition to `pendingUriProvider`, invoked only after bytes have been successfully read and base64-encoded. `ConsumerLauncherActivity` wires `onConsumed = { pendingImportUri = null }`, so a second bridge call without a fresh picker selection returns `no_file_selected`. **Explicit failure-semantics choice (as required):** a *failed* read (unreadable, too large, transient I/O error) deliberately does **not** clear the pending selection — documented in the bridge's class doc as: failures are not "the bytes were exposed," so there's no exposure reason to force re-selection, and a transient I/O failure remains retryable without sending the user back through the system picker. Only success — the point where bytes actually left the native layer — consumes the selection.
   - Tests: `selectionCanBeReadOnceThenReportsNoFileSelected` (one read succeeds, a second returns `no_file_selected`); `newlySelectedUriWorksAfterAPriorSuccessfulRead` (a fresh pick after a consumed selection works normally); `failedReadLeavesSelectionInPlaceForRetry` (a failed read does not invoke `onConsumed`, and the *same* selection can be retried once the underlying condition is resolved, without reselecting); `bridgeMethodAcceptsNoArgumentsFromJavaScript` (reflection-based structural proof that `readSelectedRecordBase64()` takes zero parameters — JS can never supply its own URI, full stop).

**3. Reviewed whether the new error-logging call could expose sensitive data.** `SafeLog.e("saf_native_read_failed", t)` was checked against `SafeLog.kt`'s actual implementation: `boundedErrorMessage()` never logs a throwable's message, cause, or stack trace — only `t.javaClass.simpleName` (e.g. `"FileNotFoundException"`), and the literal message string is passed through `PrivacyRedactor.redact()`, which additionally strips tokens/secrets/device IDs/clinical values from any string as defense in depth. Since (a) the throwable's own message (which is where a `content://` URI or filename could theoretically appear, e.g. in a `FileNotFoundException`'s text) is never logged in the first place, and (b) the literal message string this call passes is a fixed constant containing no dynamic data, this log call cannot expose content-URI path/query, patient identifiers, filenames, file contents, or bearer/session data. **No code change was required** — confirmed safe by design, not modified.

### Regression tests added
- **[ConsumerRecordImportBridgeTest.kt](android/app/src/test/java/com/healthchecker/companion/consumer/ConsumerRecordImportBridgeTest.kt)** (new, Robolectric) — 12 tests: no-selection error; the bounded-read proofs A-E above plus the declared-size fast-path test; the one-shot-on-success proofs above; the pure `classifyImportRead()` classifier exercised across every branch.
...
### Preserved security properties (explicitly re-verified, unchanged by this fix and its hardening pass)
`allowFileAccess` remains `false`; `file://` imports remain rejected (`isSafContentUri()` unchanged); no broad storage permission added (`AndroidManifest.xml` unchanged — still only `INTERNET` + Health Connect); SAF `content://` access remains narrowly scoped to exactly the user-selected URI (the bridge takes no parameter from JS at all, and per the hardening pass, that selection is now one-shot on success rather than indefinitely readable); persistable permission still only taken when the provider grants it (unchanged `try/catch(SecurityException)`); production origin lock untouched (`ConsumerOriginLock.kt`/`ConsumerOriginPolicy.kt` not modified by this fix); no cleartext/localhost fallback in the release path (unchanged); authenticated upload still required (`Authorization: Bearer` still attached, server-side `_get_authenticated_patient()` unchanged); bearer tokens/recovery secrets/signing data/file contents are never logged (confirmed against `SafeLog`'s actual implementation, see hardening item 3 above; the JS-side `console.warn` calls log only error codes and jsonContractError's existing path/status/content-type string, never the file bytes or the bridge's base64 payload); no plaintext temp file is ever written to disk (bytes exist only in process memory, now genuinely bounded to `MAX_IMPORT_BYTES` + 1 byte at the native layer — native heap briefly during the bridge read, then JS heap as a `Blob` — never persisted); `ContentResolver`-only access preserved (no filesystem APIs used anywhere in the bridge).
