...
def test_uat12_dashboard_trends_and_timeline_consumer_polish():
    ...
    assert "hc-drill-back" in snap
    assert 'CACHE_REVISION = "hc321uat12i"' in (ROOT / "service-worker.js").read_text(encoding="utf-8")
...
def test_uat12f_s24_filtered_timeline_fetch_contract_matches_snapshot_auth():
    ...
    assert 'CACHE_REVISION = "hc321uat12i"' in sw
    assert "consumer_surfaces.js?v=hc321uat12i" in html
...
def test_uat12h_compaction_and_batching_still_present():
    ...
    assert fetch_handler.find("if (isForbiddenCacheUrl(req.url)) return;") < fetch_handler.find("event.respondWith")
    assert "consumer_surfaces.js?v=hc321uat12i" in html
    assert "health_snapshot.js?v=hc321uat12i" in html
